      ...
      <div style={{ maxWidth: 440, width: '100%' }}>
        ...
        <Card>
          <div style={S.g2}>
            ...
            <div>
              <Label>Waist at navel (in)</Label>
              <Input type="number" value={form.waist} onChange={e => upd('waist', e.target.value)} placeholder="36" />
              {form.waist && !form.bodyFat && (
                <button style={{ ...S.iLink, marginTop: 4 }} onClick={() => upd('bodyFat', String(estimateBfFromWaist(parseFloat(form.waist), form.sex)))}>
    ...
    <div style={S.stack}>
      ...
      <Card>
        ...
        <div style={S.g2}>
          ...
          <div><Label>Waist at Navel (in)</Label><Input type="number" value={f.waist} onChange={e => upd('waist', e.target.value)} step="0.25" /></div>
...
function Trends({ weekly, logs, engine, currentWeek, profile }) {
  ...
  return (
    <div style={S.stack}>
          ...
          <div style={{ width: '100%', height: 200 }}>
            <ResponsiveContainer>
              <AreaChart data={weightData}>
                <defs><linearGradient id="wg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#16a34a" stopOpacity={0.3} /><stop offset="95%" stopColor="#16a34a" stopOpacity={0} /></linearGradient></defs>
                <XAxis dataKey="week" stroke="#525252" fontSize={12} /><YAxis domain={['auto', 'auto']} stroke="#525252" fontSize={12} />
      ...
      {weightData.filter(d => d.waist).length >= 2 && (
        <Card>
          <h3 style={S.cardTitle}>WAIST TREND</h3>
          <div style={{ width: '100%', height: 200 }}>
            <ResponsiveContainer>
              <AreaChart data={weightData.filter(d => d.waist)}>
                <defs><linearGradient id="wsg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#fbbf24" stopOpacity={0.3} /><stop offset="95%" stopColor="#fbbf24" stopOpacity={0} /></linearGradient></defs>
                <XAxis dataKey="week" stroke="#525252" fontSize={12} /><YAxis domain={['auto', 'auto']} stroke="#525252" fontSize={12} />
                <Tooltip contentStyle={{ backgroundColor: '#171717', border: '1px solid #333', borderRadius: 8, color: '#e5e5e5' }} />
          ...
          <div style={{ width: '100%', height: 220 }}>
            <ResponsiveContainer>
              <LineChart data={liftData}>
                <XAxis dataKey="week" stroke="#525252" fontSize={12} /><YAxis stroke="#525252" fontSize={12} />
                <Tooltip contentStyle={{ backgroundColor: '#171717', border: '1px solid #333', borderRadius: 8, color: '#e5e5e5' }} />
      ...
      {weightData.length < 2 && liftData.length < 2 && (
        <Card><p style={S.muted}>Charts appear after 2+ weekly check-ins. Complete your Sunday check-in to start.</p></Card>
      )}
        ...
        <Card>
          ...
          {engine.milestones.map((m, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', opacity: currentWeek > m.week ? 0.4 : 1, borderLeft: currentWeek >= m.week && currentWeek < (engine.milestones[i + 1]?.week || 17) ? '3px solid #16a34a' : '3px solid #333' }}>
              ...
              <span style={{ color: '#d4d4d4', flex: 1, fontSize: '0.85rem' }}>{m.weight} • {m.bf}</span>
